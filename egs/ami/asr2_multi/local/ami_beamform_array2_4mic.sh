...
do
  channels="$line "
  for ch in $bmf; do
    channels="$channels $line/audio/$line.Array2-0$ch.wav"
  done
...
# do beamforming,
echo -e "Beamforming\n"
$cmd JOB=1:$nj $wdir/log/beamform2.JOB.log \
     local/beamformit2.sh $nj JOB $numch $meetings $sdir $odir
