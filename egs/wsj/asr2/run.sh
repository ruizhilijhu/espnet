...
# multi-ctc related
sharectc=false
ctc_ws_trn=0.5_0.5 # ctc weight for each encoder during training
ctc_ws_dec=0.5_0.5 # ctc weight for each encoder during decoding
...
if [ ${stage} -le 1 ]; then
    ...
    utils/create_split_dir.pl \
        /export/b{10,11,12,13}/${USER}/espnet-data/egs/wsj/asr2/dump/${train_set}/${train_set}/delta${do_delta}/storage \
        ${feat_tr_dir}/storage
    fi
    if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d ${feat_dt_dir}/storage ]; then
    utils/create_split_dir.pl \
        /export/b{10,11,12,13}/${USER}/espnet-data/egs/wsj/asr2/dump/${train_set}/${train_dev}/delta${do_delta}/storage \
        ${feat_dt_dir}/storage
    fi
...
if [ -z ${tag} ]; then
    expdir=exp/${train_set}_${backend}_numenc${num_enc}_${conf_id}_mtlalpha${mtlalpha}_${opt}_sampprob${samp_prob}_bs${batchsize}_mli${maxlen_in}_mlo${maxlen_out}_sharectc${sharectc}_ctcws${ctc_ws_trn}
    if [ "${lsm_type}" != "" ]; then
...
if [ ${stage} -le 5 ]; then
    ...
    for rtask in ${recog_set}; do
    (
        decode_dir=decode_${rtask}_beam${beam_size}_e${recog_model}_p${penalty}_len${minlenratio}-${maxlenratio}_ctcw${ctc_weight}_ctcws${ctc_ws_dec}_rnnlm${lm_weight}_${lmtag}_batchsize${dec_batchsize}
        if [ $use_wordlm = true ]; then
