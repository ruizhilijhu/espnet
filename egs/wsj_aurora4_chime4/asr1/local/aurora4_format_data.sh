...
srcdir=data/local/aurora4_data

for x in train_si84_clean train_si84_multi test_eval92 test_0166 dev_0330 dev_1206; do
  mkdir -p data/aurora4_$x
  cp $srcdir/${x}_wav.scp data/aurora4_$x/wav.scp || exit 1;
  cp $srcdir/$x.txt data/aurora4_$x/text || exit 1;
  cp $srcdir/$x.spk2utt data/aurora4_$x/spk2utt || exit 1;
  cp $srcdir/$x.utt2spk data/aurora4_$x/utt2spk || exit 1;
  utils/filter_scp.pl data/aurora4_$x/spk2utt $srcdir/spk2gender > data/aurora4_$x/spk2gender || exit 1;
done
