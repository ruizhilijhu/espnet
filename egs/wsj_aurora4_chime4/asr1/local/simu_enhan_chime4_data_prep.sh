...
dir=`pwd`/data/local/chime4_data
mkdir -p $dir
local=`pwd`/local
utils=`pwd`/utils
odir=`pwd`/data
...
for x in $list_set; do
  mkdir -p $odir/chime4_$x
  cp ${x}_wav.scp $odir/chime4_$x/wav.scp || exit 1;
  cp ${x}.txt     $odir/chime4_$x/text    || exit 1;
  cp ${x}.spk2utt $odir/chime4_$x/spk2utt || exit 1;
  cp ${x}.utt2spk $odir/chime4_$x/utt2spk || exit 1;
done
