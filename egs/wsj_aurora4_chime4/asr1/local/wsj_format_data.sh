...
for x in train_si284 test_eval92 test_eval93 test_dev93 test_eval92_5k test_eval93_5k test_dev93_5k dev_dt_05 dev_dt_20; do
  mkdir -p data/wsj_$x
  cp $srcdir/${x}_wav.scp data/wsj_$x/wav.scp || exit 1;
  cp $srcdir/$x.txt data/wsj_$x/text || exit 1;
  cp $srcdir/$x.spk2utt data/wsj_$x/spk2utt || exit 1;
  cp $srcdir/$x.utt2spk data/wsj_$x/utt2spk || exit 1;
  utils/filter_scp.pl data/wsj_$x/spk2utt $srcdir/spk2gender > data/wsj_$x/spk2gender || exit 1;
done
