...
from espnet.asr.asr_utils import add_results_to_json, add_results_to_json_pmerr
from espnet.asr.asr_utils import CompareValueTrigger
from espnet.asr.asr_utils import get_model_conf
from espnet.asr.asr_utils import load_inputs_and_targets, load_inputs_and_targets_pmerr
from espnet.asr.asr_utils import make_batchset
from espnet.asr.asr_utils import PlotAttentionReport, PlotAttentionReportMulEnc
from espnet.asr.asr_utils import restore_snapshot
from espnet.asr.asr_utils import torch_load
from espnet.asr.asr_utils import torch_resume
from espnet.asr.asr_utils import torch_save
from espnet.asr.asr_utils import torch_snapshot
from espnet.nets.e2e_asr_th import E2E, E2E_MulEnc
from espnet.nets.e2e_asr_th import Loss, Loss_MulEnc
...
class CustomConverterPmErr(object):
    """CUSTOM CONVERTER for PM ERR"""

    def __init__(self, subsamping_factor=1, label_type='wer'):
        self.subsamping_factor = subsamping_factor
        self.ignore_id = -1
        self.label_type = label_type

    def transform(self, item):
        return load_inputs_and_targets_pmerr(item, self.label_type)
...
def train_pmerr(args):
    '''Run Performance Monitor training on Error Rate (WER or CER)'''
    # seed setting
    torch.manual_seed(args.seed)
    ...
    # specify model architecture
    pmerr = PMERR(idim, args)
    model = PmErrLoss(pmerr)
    ...
    with open(model_conf, 'wb') as f:
        logging.info('writing a model config file to ' + model_conf)
        f.write(json.dumps((idim, vars(args)), indent=4, sort_keys=True).encode('utf_8'))
    for key in sorted(vars(args).keys()):
        logging.info('ARGS: ' + key + ': ' + str(vars(args)[key]))
    ...
    # Setup a converter
    converter = CustomConverterPmErr(pmerr.subsample[0], args.label_type)
...
def recog_pmerr(args):
    ...
    # load trained model parameters
    logging.info('reading model parameters from ' + args.model)
    pmerr = PMERR(idim, train_args)
    model = PmErrLoss(pmerr)
    torch_load(args.model, model)
    pmerr.recog_args = args
...
def train_mulenc(args):
    '''Run training for multi-encoder setting'''
    # seed setting
    torch.manual_seed(args.seed)
    ...
    # specify model architecture
    e2e = E2E_MulEnc(idim, odim, args)
    model = Loss_MulEnc(e2e, e2e.mtlalpha, e2e.ctc_ws_trn)
    ...
    # Setup a converter
    converter = CustomConverter(e2e.subsample[0][0])
    ...
    if args.num_save_attention > 0 and args.mtlalpha != 1.0:
        ...
        if hasattr(model, "module"):
            ...
            att_vis_fn = model.predictor.calculate_all_attentions
        trainer.extend(PlotAttentionReportMulEnc(
            att_vis_fn, data, args.outdir + "/att_ws",
            converter=converter, device=device), trigger=(1, 'epoch'))
...
def recog_mulenc(args):
    '''Run recognition for multi-encoder setting'''
    # seed setting
    torch.manual_seed(args.seed)
    ...
    # load trained model parameters
    logging.info('reading model parameters from ' + args.model)
    e2e = E2E_MulEnc(idim, odim, train_args)
    model = Loss_MulEnc(e2e, e2e.mtlalpha, e2e.ctc_ws_trn)
    torch_load(args.model, model)
    e2e.recog_args = args
